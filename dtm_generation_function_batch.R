...
################ FUNCTION SETTINGS ###################
######################################################
input.filepath <- "D:/OREF_tls_microclimate_project/point_cloud_data/las_files/las_local_coord/"
...
filenames <- gsub('.{0,4}$', '', filenames)# remove file ending from file names
...
getDTM <- function(input, output) {
 ...
 #clip_las <- clip_circle(las, 0, 0, buffer.size) # comment out whichever
  ...
  writeRaster(dtm_tin, 
              paste0(output.filepath, output,  "_DTM_", dtm.res, "_", buffer.size, ".tif"), 
              overwrite = T)
}
